...
bool hash_redimensionar(hash_t* hash, size_t nuevo_tam){
	...
	for(size_t i = 0; i < viejo_tam; i ++){
		if(datos_aux[i]->estado == ocupado){
			char* aux = malloc(sizeof(char) * (strlen(datos_aux[i]->clave) + 1));
			if(aux == NULL){
				return false;
			}
			strcpy(aux, datos_aux[i]->clave);
			if(!hash_guardar(hash, aux, datos_aux[i]->dato)){
				for(size_t x = 0; x < hash->tam; x ++){
					if(hash->dest != NULL && hash->datos[x]->estado != vacio){
						hash->dest(hash->datos[x]->dato);
						free(hash->datos[x]->clave);
					}
					free(hash->datos[x]);
				}				
				hash->datos = datos_aux;
				return false;
			}
			free(aux);
		}
	}
	for(size_t j = 0; j < viejo_tam; j ++){
		if(datos_aux[j]->estado == ocupado){
			free(datos_aux[j]->clave);
		}
		free(datos_aux[j]);
	}
	free(datos_aux);
...
bool hash_guardar(hash_t *hash, const char *clave, void *dato){
	...
	if(hash->datos[pos]->estado != ocupado){
		...
		size_t lugar = pos;
		while(hash->datos[lugar]->estado == ocupado){
			if((strcmp(hash->datos[lugar]->clave, clave)) == 0) {
				if(hash->dest != NULL){
					hash->dest(hash->datos[lugar]->dato);
				}
				hash->datos[lugar]->dato = dato;
				return true;
			}
			else if(hash->datos[lugar]->estado == borrado){
				if(hash->dest != NULL){
					hash->dest(hash->datos[lugar]->dato);
				}
				char* aux = malloc(sizeof(char) * (strlen(clave) + 1));
				if(aux == NULL){
					...
					strcpy(aux, clave);
				hash->datos[lugar]->clave = aux;
				hash->datos[lugar]->dato = dato;
				hash->datos[lugar]->estado = ocupado;
				hash->ocupado ++;
				return true;
			}
			lugar ++;
...
void *hash_obtener(const hash_t *hash, const char *clave){
	...
	if(hash->datos[pos]->estado == ocupado){
		if(strcmp(hash->datos[pos]->clave, clave) == 0){
...
//los borrados suman tambien a los hash->ocupado, no hago hash->ocupado --
void *hash_borrar(hash_t *hash, const char *clave){
	...
	while( hash->datos[lugar]->estado != vacio){
		...
		if(hash->datos[lugar]->estado == ocupado){
			if( strcmp(hash->datos[lugar]->clave, clave) == 0 ){
				void* dato = hash->datos[lugar]->dato;
				hash->datos[lugar]->dato = NULL;
				free(hash->datos[lugar]->clave);
				hash->datos[lugar]->estado = borrado;
				hash->ocupado --;
				return dato;
			}
		}
